...
contingency = pd.crosstab(y, clusters, rownames=["True"], colnames=["Cluster"])
print("Cluster vs. True Label:")
print(contingency)

out_dir = os.path.abspath(os.path.join(__file__, os.pardir, os.pardir))
contingency.to_csv(os.path.join(out_dir, "kmeans_contingency.csv"))
print(f"Saved contingency table to {out_dir}/kmeans_contingency.csv")
...
for cl in np.unique(clusters):
    mask = (clusters == cl)
    plt.scatter(X2[mask,0], X2[mask,1], label=f"Cluster {cl}", alpha=0.6)
plt.xlabel("PC1")
plt.ylabel("PC2")
...
print(f"Saved PCA-cluster scatter to {out_dir}/kmeans_pca.png")
